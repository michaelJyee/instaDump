...
module.exports = {
  'lets test some functions': function (test) {
      ...
      .wait(4000)
      .toFrame('.hiFrame')
      .type('input[name="username"]', data.instaUsername2)
    ...
    for(var i=0; i<data.hash1.length; i++){
      ...
      test.wait(2500)
      var random1 = Math.floor((Math.random() * data.hash2.length) + 0);
      var random2 = Math.floor((Math.random() * data.hash2.length) + 0);
      test.type('input[placeholder="Leave a comment..."]', "#"+data.hash1[i]+"_"+data.hash2[x])
      test.wait(2500)
      test.sendKeys('body', '\uE007')
      test.wait(2000)
      }
